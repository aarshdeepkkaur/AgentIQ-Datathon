...
| **Repository** | [github.com/aarshdeepkkaur/AgentIQ-Datathon](https://github.com/aarshdeepkkaur/AgentIQ-Datathon) |
| **Live app** | Frontend on Vercel, backend on Render at `https://agentiq-datathon.onrender.com` — see [Deployment](#deployment) |
...
- [Data Cleaning Evidence](#data-cleaning-evidence)
...
The Vite dev server proxies `/api` to `http://127.0.0.1:8000`, so the frontend needs no backend URL configuration in development. In production, the frontend uses `VITE_API_URL` to talk directly to the deployed Render backend — see [Deployment](#deployment).
...
| Groq API key | Required for the LLM-backed AI agent mode. The agent uses Groq's `llama-3.3-70b-versatile` model; see [Using the AI Agent](#using-the-ai-agent). |
...
`requirements.txt` includes the backend runtime dependencies used by the application, including `motor`, `pymongo`, and `openpyxl`.

- `motor` and `pymongo` are needed by `lib/db.py`.
- `openpyxl` is needed by the weather-sheet step in `notebooks/01_data_cleaning.py`.

> If pip reports an encoding error while reading `requirements.txt`, re-save the file as UTF-8 and retry.

### 4. Create the `.env` file

Create a file named `.env` in the project root. The backend calls `os.environ["MONGO_URL"]` and `os.environ["DB_NAME"]` directly, so these two values are required for the backend to start.
...
The Groq API key is required for the LLM-backed agent:

```env
GROQ_API_KEY=your_groq_api_key
```

Other optional variables, with working defaults if omitted:
...
> **Security note:** `.env` is listed in `.gitignore` and must never be committed. Use your own credentials — real API keys, MongoDB connection strings, and other secrets must never be committed to GitHub.
...
Local development uses the npm scripts defined in `frontend/package.json`:
...
|---|---|---|
| `rules` | Fallback mode | Intent detection over the question, then a fixed pandas analysis. Recognized intents include MSP risk, top mandis by modal price, crop comparison, route health, and weather impact. |
| `llm` | When `GROQ_API_KEY` is configured | The agent uses Groq's `llama-3.3-70b-versatile` model to generate pandas analysis code over the already-loaded agricultural datasets. The generated code runs in a restricted execution environment with a timeout, and the model then produces a natural-language explanation of the result. |

The LLM-backed mode uses Groq and `llama-3.3-70b-versatile`. If the Groq key is not configured, or if the LLM path fails, the agent uses the rule-based path instead. Every response carries a `mode` field so you can see which path produced it.

> The agent does not run entirely offline in `llm` mode — that mode requires a live call to Groq's API.

The generated pandas code is restricted to analysis over the already-loaded datasets. File, network, OS, and other disallowed operations are blocked, and code execution has a timeout.
...
`/api/dashboard` and `/api/alerts` accept `crop`, `date_from`, and `date_to` query parameters; `/api/dashboard` also accepts `refresh`. An unrecognized crop returns `404` with the list of supported crops.

Full request and response schemas are browsable at `http://127.0.0.1:8000/docs` locally, or against the production backend's `/docs` route.
...
- Missing or unrecognized units, left as `NaN` rather than guessed
...
Column-level documentation is in `DATA_DICTIONARY.md`. Field notes on the raw files are in `data/raw/track3_dataset_notes.txt`.
...
The full breakdown, including how each duplicate count was verified, is in the Data Cleaning Report. Field-level definitions are in the Data Dictionary.

---

## Deployment

The live app follows this architecture:

```
Vercel (React frontend)
        │
        ▼
Render (FastAPI backend) — https://agentiq-datathon.onrender.com
        │
        ▼
MongoDB / agricultural data
        │
        ▼
Groq AI agent (llama-3.3-70b-versatile)
```

### Backend (Render)

The FastAPI backend (`server.py`, project root) is deployed on Render, built from `requirements.txt`, and started with:
...
```
https://agentiq-datathon.onrender.com
```

Render's environment variables mirror the required local `.env` configuration:

- `MONGO_URL`
- `DB_NAME`
- `GROQ_API_KEY`
- `CORS_ORIGINS`
- `AGENTIQ_DATA_REPO_BASE`

`MONGO_URL`, `DB_NAME`, and `GROQ_API_KEY` are required for the deployed application. These values are configured in the Render dashboard and are never committed to the repository.

The Render service exposes the FastAPI API under:

```
https://agentiq-datathon.onrender.com/api/
```

The API documentation is available at:

```
https://agentiq-datathon.onrender.com/docs
```

### Frontend (Vercel)

The Vite/React frontend in `frontend/` is deployed on Vercel. In production it is pointed at the Render backend through:

```env
VITE_API_URL=https://agentiq-datathon.onrender.com/api
```

The frontend API layer uses `VITE_API_URL` when it is available and falls back to the relative `/api` prefix:

```
VITE_API_URL || "/api"
```

Locally, `VITE_API_URL` does not need to be set — the frontend continues to use the Vite dev-server `/api` proxy to `http://127.0.0.1:8000` (see [Architecture](#architecture)).

In production, `VITE_API_URL` is configured as an environment variable in the Vercel project settings rather than committed to the repository.

> `VITE_API_URL` contains only the public backend URL and does not contain API keys or other secrets. Real API keys, MongoDB connection strings, and other secrets must never be committed to GitHub or exposed in the frontend build — they are set as environment variables in Render/Vercel only.
...
│   └── tsconfig*.json
├── lib/
│   ├── agent_llm.py               # LLM-backed agent path (Groq)
...
| Groq | LLM backing the `llm` agent mode (`llama-3.3-70b-versatile`) |
...
| MongoDB | Agent chat history persistence |
| Groq | LLM inference for the AI agent |
...
- **LLM mode requires Groq.** The LLM-backed agent uses `GROQ_API_KEY` and the `llama-3.3-70b-versatile` model. If the key is unavailable or an LLM request fails, the application falls back to the rule-based path.
- **Generated code is restricted.** In LLM mode, Groq generates pandas analysis code that runs against already-loaded datasets inside a restricted execution namespace. File, network, OS, and other disallowed operations are blocked.
