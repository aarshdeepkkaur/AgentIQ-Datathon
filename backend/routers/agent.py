...
from models.agent import AgentAnswer, AgentAskRequest, AgentChartPoint
...
async def ask_agent(request: AgentAskRequest) -> AgentAnswer:
    ...
    requested = canonical_crop(request.crop_name)
    crop = detect_crop(query, requested if requested in CROPS else "Wheat")
    data = build_dashboard(bundle, crop)
    ...
    return AgentAnswer(answer=answer, intent=intent, source=data.source, grounded_at=data.fetched_at, evidence=evidence, chart=chart)
