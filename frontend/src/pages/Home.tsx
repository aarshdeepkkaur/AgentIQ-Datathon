...
import type { CropName, DashboardResponse, RiskLevel } from "@/data/types";
import { formatDate, formatInr, formatQtl } from "@/data/types";
...
export default function Home() {
  ...
  const dashboardQuery = useQuery<DashboardResponse>({
    queryKey: ["dashboard", crop],
    queryFn: () => apiGet<DashboardResponse>(`/dashboard?crop=${encodeURIComponent(crop)}`),
    staleTime: 5 * 60 * 1000,
    retry: 1,
  ...
  return <div className="reference-skin min-h-screen bg-[#07111F] text-[#E2E8F0]" data-testid="agentiq-dashboard">
    ...
    <main className="min-h-screen lg:pl-64 xl:pl-72" data-testid="dashboard-main-content"><div className="mx-auto max-w-[1720px] space-y-6 p-4 pt-20 sm:p-6 sm:pt-20 lg:space-y-7 lg:p-8 lg:pt-8">
      <HeaderFilterBar crop={crop} crops={data.crops as CropName[]} states={data.states} mandiNames={allRows.map((row) => row.mandi_name)} dateFrom={data.totals.date_from} dateTo={data.totals.date_to} state={state} mandi={mandi} risk={risk} search={search} onCropChange={handleCropChange} onStateChange={setState} onMandiChange={handleMandiChange} onRiskChange={setRisk} onSearchChange={setSearch} onReset={resetFilters} dataSource={dataSource} isRefreshing={dashboardQuery.isFetching} />
      ...
      <MandiDataTable rows={rows} totalCount={allRows.length} search={search} onSearchChange={setSearch} selectedMandiId={selectedMandiId} onSelectMandi={setSelectedMandiId} />
