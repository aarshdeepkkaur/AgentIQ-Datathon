...
import type { AgentAnswer, CropName } from "@/data/mockData";
import { apiPost } from "@/lib/api";

interface AskAgentIqDrawerProps { open: boolean; onClose: () => void; crop: CropName; }
interface Message { role: "user" | "agent"; text: string; evidence?: string[]; }

const SUGGESTIONS = ["Show mandis where this crop is below MSP", "Which mandi has the lowest transit time?", "What is the current weather signal?"];
...
export function AskAgentIqDrawer({ open, onClose, crop }: AskAgentIqDrawerProps) {
  ...
  const askMutation = useMutation<AgentAnswer, Error, string>({
    ...
    onSuccess: (result) => setMessages((current) => [...current, { role: "agent", text: result.answer, evidence: result.evidence }]),
